...
Explicit `-a`/`--agent` always overrides both, and `-r`/`--resume` bypasses both so the thread's original agent is restored. See [Switch agents](/oss/javascript/deepagents/cli/overview#switch-agents) for the full precedence chain.
...
<ResponseField name="DEEPAGENTS_CLI_LANGSMITH_PROJECT" type="string" post={["optional"]}>
    Override the LangSmith project name for agent traces. See [Tracing with LangSmith](/oss/javascript/deepagents/cli/overview#tracing-with-langsmith).
</ResponseField>
