    ...
    <Accordion title="sse">
        **Required:** `type: "sse"`, `url`. **Optional:** `headers`, `auth`, plus the shared [tool-filter fields](#tool-filtering).
        ...
        <ResponseField name="auth" type='"oauth"'>
            Set to `"oauth"` to drive an OAuth login flow with `deepagents mcp login` instead of supplying an `Authorization` header. Cannot be combined with an `Authorization` header. See [OAuth login](#oauth-login).
        </ResponseField>
    </Accordion>
