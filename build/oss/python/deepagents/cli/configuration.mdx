...
The manager lists every provider the CLI knows about, marks the ones that already have a key, and lets you paste, rotate, or clear credentials inline. Each provider renders as one of:
...
Explicit `-a`/`--agent` always overrides both, and `-r`/`--resume` bypasses both so the thread's original agent is restored. See [Switch agents](/oss/python/deepagents/cli/overview#switch-agents) for the full precedence chain.
...
<Tip>
    For one-off adjustments without editing `config.toml`, pass a JSON object via `--model-params` at launch or mid-session with `/model`. CLI flags take highest priority over the config file. See [Model parameters](/oss/python/deepagents/cli/providers#model-parameters) on the providers page for syntax and provider-specific examples.
</Tip>
