...
<Steps>
    ...
    <Step title="Add provider credentials" icon="key">
        The CLI works with any LLM that supports tool calling. Use `/auth` inside the TUI to paste an API key for any provider — see [Provider credentials](/oss/python/deepagents/cli/configuration#provider-credentials) for the full flow and storage details. Environment variables (e.g. `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, ...) work for CI and headless runs.

        For the full list of providers and the credentials they expect, see [Providers](#providers).
    </Step>
...
<AccordionGroup>
    <Accordion title="Slash commands" icon="slash">
        ...
        - `/auth` - Manage stored API keys for model providers. See [Provider credentials](/oss/python/deepagents/cli/configuration#provider-credentials) for details
...
CLI flags are session-only. `--model-params` cannot be combined with `--default`. For persistent provider-level defaults, per-model overrides, and provider-specific examples, see [Model parameters](/oss/python/deepagents/cli/providers#model-parameters) on the providers page.
...
The frontmatter requires `name` and `description` (same as the [`SubAgent` dictionary spec](/oss/python/deepagents/subagents#subagent-dictionary-based)). The markdown body becomes the subagent's `system_prompt`. In addition to the base spec, `AGENTS.md` files support an optional `model` frontmatter field that overrides the main agent's model for this subagent. Uses the `provider:model-name` format (e.g., `anthropic:claude-opus-4-6`, `openai:gpt-5.5`). Omit to inherit the main agent's model.
...
Extend the CLI with tools from external [MCP (Model Context Protocol)](https://modelcontextprotocol.io/) servers. Place a `.mcp.json` at your project root and the CLI discovers it automatically. Servers that require OAuth (Slack, GitHub, Linear, Notion, …) can be authenticated once with `deepagents mcp login <server>`. See the [MCP tools guide](/oss/python/deepagents/cli/mcp-tools) for configuration format, OAuth login, auto-discovery, and troubleshooting.
...
<AccordionGroup>
    <Accordion title="Command-line options" icon="flag">
        | Option                 | Description                                                 |
        |------------------------|-------------------------------------------------------------|
        | `-a`, `--agent NAME`   | Use a named agent with its own memory. See [Switch agents](#switch-agents) |
    ...
    <Accordion title="CLI commands" icon="terminal">
        ...
        | `deepagents mcp login NAME [--config PATH]` | Run the OAuth login flow for an MCP server marked `auth: "oauth"`. See [MCP tools](/oss/python/deepagents/cli/mcp-tools#oauth-login) |
        | `deepagents deploy`                  | Deploy your agent to LangSmith. See [Deploy with the CLI](/oss/python/deepagents/deploy) |
