...
- Access to configure OpenCode plugins in `opencode.json` or `~/.config/opencode/opencode.json`.
...
Run OpenCode as usual. Completed user turns are sent to the configured LangSmith project.

## Configure tracing

Tracing is disabled by default. Enable it with `TRACE_TO_LANGSMITH=true` or with a LangSmith config file.
...
Trace completion is based on OpenCode `step-finish` events. The plugin also flushes pending trace batches when the OpenCode server instance is disposed.
