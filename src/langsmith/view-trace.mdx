...
For SDKs not listed under [Supported integrations](#supported-integrations), such as the LangSmith Go or Java SDK, configure your spans manually so the Messages view can render your traces correctly. Two things are required:
...
For OpenTelemetry-based traces, LangSmith reads inputs and outputs from the raw API request and response captured by your instrumentation library. No message reformatting is required. For outer spans (chain and tool runs), set inputs and outputs using the `gen_ai.prompt` and `gen_ai.completion` span attributes.
...
#### Example: Go with Anthropic

The following example uses the LangSmith Go SDK's OTel tracer with `traceanthropic` auto-instrumentation. The `traceanthropic`-wrapped client automatically captures each `messages.New` call as an LLM span. Tool spans are created manually after the LLM call, satisfying the adjacency requirement.
